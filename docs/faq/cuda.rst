...
#. Via `UCX <https://www.openucx.org/>`_.
   ...
   CUDA support, it is important to ensure that UCX itself is build
   with CUDA support.

   .. error:: JMS Would be best to point to UCX docs that show how
              to build with CUDA support.  The old FAQ says you need
...
#. Via internal Open MPI CUDA support
   ...
   .. error:: JMS Is this still relevant?  Or should Open MPI
              *always* be built with
              ``--with-cuda=/usr/local/cuda``, regardless of whether
              you use UCX+CUDA or not?
...
.. error:: JMS Is the above paragraph still correct?
...
* The shared memory (``sm``), CUDA-ized shared memory (``smcuda``) and
  TCP (``tcp``) BTLs with the OB1 (``ob1``) PML.
...
feature. To enable this, users will need to set ``PSM2_GPUDIRECT`` to
...
* The ``smcuda`` BTL has three MCA parameters related to the use of
  ...
  user can now turn it off if they want.
...
.. error:: JMS Is this list correct?

.. error:: JMS Combine this with the UCX supported question.
...
.. error:: JMS Combine this with the UCX unsupported question.
...
.. error:: JMS Combine this with the "supported" question, above.
...
.. error:: JMS Combine this with the "unsupported" question, above.
...
.. error:: JMS Are Intel/OPA references still correct?
