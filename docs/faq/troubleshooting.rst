...
* The plugin is for a different version of Open MPI.  :ref:`See this
  FAQ entry <faq-building-open-mpi-install-overwrite>` for an
...
* An application is trying to manually dynamically open ``libmpi`` in
  ...
  * Use the ``--disable-dlopen`` or ``--disable-mca-dso`` options to
    Open MPI's ``configure`` script (see :ref:`this FAQ entry
    <faq-building-open-mpi-avoid-dso-label>` for more details on these
...
.. error:: JMS Need to update this with PMIx debugger info.
